...
class HeatMapView: UIView {
    ...
    @objc func setData(_ x1:UnsafeMutablePointer<Double>, x2:UnsafeMutablePointer<Double>, y:UnsafeMutablePointer<Double>, size:Int){
        ...
        for i in 0..<trainx.count{
            ...
            var path = UIBezierPath(ovalIn: CGRect(x: Double(halfWidth * trainx[i][0] + halfWidth)-1, y: Double(halfHeight - halfHeight * trainx[i][1])-1, width: 7.0, height: 7.0))  //减1是为了圆心和半径5的圆匹配
            pathLayer.path = path.cgPath
            pathLayer.fillColor = white.cgColor
            dataLayer.addSublayer(pathLayer)
            
            pathLayer = CAShapeLayer()
            path = UIBezierPath(ovalIn: CGRect(x: Double(halfWidth * trainx[i][0] + halfWidth), y: Double(halfHeight - halfHeight * trainx[i][1]), width: 5.0, height: 5.0))
            pathLayer.path = path.cgPath
            if trainy[i] > 0{
    ...
    @objc func setTestData(_ x1:UnsafeMutablePointer<Double>, x2:UnsafeMutablePointer<Double>, y:UnsafeMutablePointer<Double>, size:Int){
        ...
        for i in 0..<testx.count{
            ...
            //先添加黑底圆，再添加颜色圆
            var pathLayer = CAShapeLayer()
            var path = UIBezierPath(ovalIn: CGRect(x: Double(halfWidth * testx[i][0] + halfWidth)-1, y: Double(halfHeight - halfHeight * testx[i][1])-1, width: 7.0, height: 7.0))  //减1是为了圆心和半径5的圆匹配
            pathLayer.path = path.cgPath
            pathLayer.fillColor = black.cgColor
            dataLayer.addSublayer(pathLayer)
            
            pathLayer = CAShapeLayer()
            path = UIBezierPath(ovalIn: CGRect(x: Double(halfWidth * testx[i][0] + halfWidth), y: Double(halfHeight - halfHeight * testx[i][1]), width: 5.0, height: 5.0))
            pathLayer.path = path.cgPath
            if testy[i] > 0{
                pathLayer.fillColor = blue.cgColor
            }else{
